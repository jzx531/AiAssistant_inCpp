...
HttpServer::HttpServer(int port,const std::string & name,bool useSSL,
        muduo::net::TcpServer::Option option):listenAddr_(port),
                           ...
                           muduo::Timestamp receiveTime)
{
    try
    {
        ...
        if(!context -> parseRequest(buf,receiveTime)) // 解析请求报文失败
        {
            // 如果解析http报文过程中出错
        ...
        if (context->gotAll())
        {
            onRequest(conn, context->request()); // 处理请求报文
            context->reset(); // 重置上下文
        }
    }
...
void HttpServer::onRequest(const muduo::net::TcpConnectionPtr &conn, const HttpRequest &req)
{
    ...
    //可以给response设置一个成员,判断请求是否是文件,如果是文件设置true
    muduo::net::Buffer buf;
    response.appendToBuffer(&buf); //将响应报文封装到buf缓冲区中
    // 打印完整的响应内容用于调试
